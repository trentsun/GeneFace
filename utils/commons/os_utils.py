# import os
# import subprocess


# def link_file(from_file, to_file):
#     subprocess.check_call(
#         f'ln -s "`realpath --relative-to="{os.path.dirname(to_file)}" "{from_file}"`" "{to_file}"', shell=True)


# def move_file(from_file, to_file):
#     subprocess.check_call(f'mv "{from_file}" "{to_file}"', shell=True)


# def copy_file(from_file, to_file):
#     subprocess.check_call(f'cp -r "{from_file}" "{to_file}"', shell=True)


# def remove_file(*fns):
#     for f in fns:
#         subprocess.check_call(f'rm -rf "{f}"', shell=True)
import os
import shutil
import subprocess
import platform

def link_file(from_file, to_file):
    if platform.system() == "Windows":
        subprocess.check_call(f'mklink "{to_file}" "{from_file}"', shell=True)
    else:
        subprocess.check_call(f'ln -s "{os.path.relpath(from_file, os.path.dirname(to_file))}" "{to_file}"', shell=True)

def move_file(from_file, to_file):
    shutil.move(from_file, to_file)

def copy_file(from_file, to_file):
    shutil.copytree(from_file, to_file)

def remove_file(*fns):
    for f in fns:
        if os.path.isdir(f):
            shutil.rmtree(f)
        elif os.path.isfile(f):
            os.remove(f)
